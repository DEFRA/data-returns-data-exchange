package uk.gov.defra.datareturns.service.csv.fields;
...
public enum ReturnPeriodFormat {
    ...
    ReturnPeriodFormat(final String regex, final String replacement) {
     ...
     */
    public static ReturnPeriodFormat from(final String input) {
        if (input != null) {
            for (final ReturnPeriodFormat fmt : values()) {
     ...
     */
    public static String toStandardisedFormat(final String input) {
        final ReturnPeriodFormat fmt = from(input);
        String value = input;
        if (fmt != null) {
