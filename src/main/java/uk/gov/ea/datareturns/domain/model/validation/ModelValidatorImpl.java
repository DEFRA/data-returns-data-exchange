 ...
 */
public class ModelValidatorImpl<T extends MVO> implements ModelValidator<T> {
    private final Map<String, FieldMapping> fieldNameToBeanMap;

    /** hibernate validator instance */
    private final Validator validator;
    private final Class<T> measurementValidationObjectClass;
     ...
     */
    @Inject
    public ModelValidatorImpl(final Validator validator, final Class<T> measurementValidationObjectClass) {
        this.validator = validator;
        this.measurementValidationObjectClass = measurementValidationObjectClass;
        fieldNameToBeanMap = FieldMapping.getFieldNameToBeanMap(measurementValidationObjectClass);
    }
    ...
    private List<ValidationErrorField> getErrorDataFromFields(T record,
        ...
        if (fieldsForValidation != null) {
            for (FieldDefinition field : fieldsForValidation) {
                FieldMapping fieldMapping = fieldNameToBeanMap.get(field.getName());
