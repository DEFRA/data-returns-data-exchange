package uk.gov.ea.datareturns.actuator.mvc;
...
import uk.gov.ea.datareturns.actuator.EndpointsListEndpoint;
...
public class EndpointsListMvcEndpoint extends EndpointMvcAdapter {
    ...
    @Inject
    public EndpointsListMvcEndpoint(EndpointsListEndpoint delegate) {
        super(delegate);
    ...
    @ResponseBody
    public Set<EndpointResource> filter(@RequestParam(required = false) Boolean enabled,
            @RequestParam(required = false) Boolean sensitive) {
        Predicate<Endpoint> isEnabled = endpoint -> matches(endpoint::isEnabled, ofNullable(enabled));
        Predicate<Endpoint> isSensitive = endpoint -> matches(endpoint::isSensitive, ofNullable(sensitive));
    ...
    public static class EndpointResource extends ResourceSupport {
        ...
        public EndpointResource(String managementContextPath, Endpoint endpoint) {
            this.managementContextPath = managementContextPath;
            this.endpoint = endpoint;

            if (endpoint.isEnabled()) {
                UriComponentsBuilder path = ServletUriComponentsBuilder.fromCurrentServletMapping()
