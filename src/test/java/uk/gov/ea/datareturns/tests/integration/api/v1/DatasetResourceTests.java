...
public class DatasetResourceTests extends AbstractDataResourceTests {
    ...
    public void testListDatasets() {
        ...
        for (String datasetId : datasets) {
            datasetRequest(HttpStatus.CREATED).putDataset(TEST_EA_ID,
                    datasetId, null);
        }

        // Test list shows these datasets
        ResponseEntity<EntityReferenceListResponse> list = datasetRequest(HttpStatus.OK).listDatasets(TEST_EA_ID);
    ...
    private void executeConditionalGetRequestTest(HttpStatus expected,
        ...
        ResponseEntity<DatasetEntityResponse> createResponse = createTestDataset(TEST_EA_ID);
        ...
        ResponseEntity<DatasetEntityResponse> getResponse = datasetRequest(expected)
                .withHeaders(headers)
                .getDataset(TEST_EA_ID, original.getId());
    ...
    private void executeConditionalPutRequestTest(HttpStatus expected,
        ...
        ResponseEntity<DatasetEntityResponse> updateResponse = datasetRequest(expected).withHeaders(headers)
                .putDataset(TEST_EA_ID, original.getId(), props);
    ...
    private void executeConditionalDeleteRequestTest(HttpStatus expected,
        ...
        // First PUT a new dataset
        ResponseEntity<DatasetEntityResponse> createResponse =
        ...
        ResponseEntity<?> deleteResponse = datasetRequest(expected).withHeaders(headers).deleteDataset(
                TEST_EA_ID, original.getId());
        if (deleteResponse.getStatusCode().is2xxSuccessful()) {
            Assert.assertNull(deleteResponse.getBody());
            // Check that the record has actually been deleted
            datasetRequest(HttpStatus.NOT_FOUND).getDataset(TEST_EA_ID, original.getId());
        } else {
    ...
    private ResponseEntity<MultiStatusResponse> executeBatchUpdateRequestTest(Map<String, MultiStatusResponse.Response> originalDatasets,
        ...
        ResponseEntity<MultiStatusResponse> responses = datasetRequest(HttpStatus.MULTI_STATUS).postDatasets(TEST_EA_ID, new BatchDatasetRequest(requests));
